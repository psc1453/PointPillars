...
class Head(nn.Module):
    def __init__(self, in_channel, n_anchors, n_classes):
        ...
        self.conv_all = nn.Conv2d(in_channel, n_anchors * (n_classes + 7 + 2), 1, bias=False)
    ...
    def forward(self, x):
        ...
        bbox = self.conv_all(x)
        bbox_cls_pred = bbox[:, :self.n_anchors * self.n_classes]
        bbox_pred = bbox[:, self.n_anchors * self.n_classes: self.n_anchors * (self.n_classes + 7)]
        bbox_dir_cls_pred = bbox[:, self.n_anchors * (self.n_classes + 7):]
